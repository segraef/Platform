...
@description('Required. The structure of the management groups')
param mgStructure array
...
resource noop 'Microsoft.Management/managementGroups@2020-05-01' = if (false) {
  scope: tenant()
  name: 'noop'
  properties: {
    details: {
...
resource mgStructure_mgLoop_name 'Microsoft.Management/managementGroups@2020-05-01' = [for item in mgStructure: if (item.parentId != '/') {
  scope: tenant()
  name: item.name
  properties: {
    ...
    details: {
      parent: {
        id: '/providers/Microsoft.Management/managementGroups/${item.parentId}'
      }
    }
  }
  dependsOn: [
    ((contains(item, 'parentNotManagedInThisTemplate') && item.parentNotManagedInThisTemplate) ? 'noop' : item.parentId)
  ]
}]

module MGRBAC_mgStructure './module/nested_MGRBAC_mgStructure.bicep' = [for (item, i) in mgStructure: {
  name: 'MGRBAC-${(empty(mgStructure) ? 'dummy' : i)}'
  params: {
    MGConfig: item
  ...
  dependsOn: [
    mgStructure_mgLoop_name
  ]
}]

@description('Number of management groups considered in the deployment')
output managementGroupCount int = length(mgStructure)
